...
@ecosystem_tests.command(name='blueprint-linting',
                         short_help='validate blueprints in a repo.')
@ecosystem_tests.options.access_token
@ecosystem_tests.options.repo_name
@ecosystem_tests.options.directory
def blueprint_linting(access_token=None, repo_name=None, directory=None):
    ...
    repo = Repo.clone_from(git_repo.clone_url, os.path.join(os.getcwd(), directory))
    ...
    try:
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file_name in file:
                    ...
                    p = subprocess.Popen(full_command.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE) 
        ...
        git_repo.create_git_ref(ref='refs/heads/'+ branch_name, sha=sb.commit.sha)
        repo.git.pull()
    ...
    if not status == status_no_change:
        ...
        title = "wave bear"
        body = "testing"
        pr = git_repo.create_pull(title=title, body=body, head=branch_name, base=source_branch)
